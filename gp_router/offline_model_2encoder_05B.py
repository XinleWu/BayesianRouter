...
class RewardDiffPredictor(Qwen2PreTrainedModel):
    def __init__(self, config, num_reward_models: int):
        ...
        # self.bt_bias = nn.Parameter(torch.zeros(num_reward_models))

        # self.bt_scale = nn.Parameter(torch.tensor(1.0))
        # self.bt_logtau = nn.Parameter(torch.tensor(0.0))
        # self.bt_scale = nn.Parameter(torch.ones(num_reward_models))
        # self.bt_logtau = nn.Parameter(torch.tensor(math.log(math.exp(1.0) - 1.0)))  # softplus(x)=1 -> x=log(e^1-1)
    ...
    def load_qwen_backbone(self, model_path: str):
        backbone = Qwen2Model.from_pretrained(model_path)
    ...
    def forward(
        ...
        # cos = torch.matmul(h_norm, bt_vec_norm.t())  # (B, K)
        # raw = cos * self.bt_scale.unsqueeze(0) + self.bt_bias.unsqueeze(0)
        # tau = F.softplus(self.bt_logtau) + 1e-6
        # bt_scores = raw / tau
